...
function PlayMP:CreatFrame( frameTitle, uniqueName )
	...
	function PlayMP:AddOption( name, uniqueName, image, func )
		...
		if uniqueName == "systemInfo" or uniqueName == "updateLog" then
			if PlayMP.info.newer_version != nil and tonumber(PlayMP.info.newer_version) > tonumber(PlayMP.info.version) then
				f.Notion(true)
			end
		end
	...
	function PlayMP:AddCheckBox( panel, func, text, def, instant, vaild, unique )
		...
		if instant != nil and instant == true then
			...
			table.Vaild = tobool(PlayMP:GetSetting( def ))
		end
		...
		function TogDoClick:DoClick()
			...
			if instant != nil and instant == true then
				...
				PlayMP:ChangeSetting( def, table.Vaild )
	...
	function PlayMP:MainMenu()
		...
		VolumeSlider.ValueChanging = function(  )
			PlayMP.Player:set_vol( VolumeSlider:GetValue() )
		...
		local playtimetime = math.Round( PlayMP.Player.Cur_play_time )
		playtime.Think = function()
			if math.Round( PlayMP.Player.Cur_play_time ) != playtimetime then
				playtimetime = math.Round( PlayMP.Player.Cur_play_time )
		...
		local remaintimetime = math.Round( PlayMP.Player.Cur_Media_Length - PlayMP.Player.Cur_play_time )
		remaintime.Think = function()
			if math.Round(PlayMP.Player.Cur_Media_Length - PlayMP.Player.Cur_play_time) != remaintimetime then
				remaintimetime = math.Round( PlayMP.Player.Cur_Media_Length - PlayMP.Player.Cur_play_time )
		...
		seekto:SetNum( PlayMP.Player.Cur_play_time / PlayMP.Player.Cur_Media_Length )
		seekto.ValueChanged = function( d ) 
			PlayMP:DoSeekToVideo( d * PlayMP.Player.Cur_Media_Length )
			seektoTimeView:AlphaTo(0,1,1)
		end
		seekto.ValueChanging = function( d )
			seektoTimeView:SetAlpha(255)
			surface.SetFont( "Default_PlaymusicPro_Font" )
			local w, h = surface.GetTextSize(string.ToMinutesSeconds(d * PlayMP.Player.Cur_Media_Length))
			seektoTimeViewText:SetText(string.ToMinutesSeconds(d * PlayMP.Player.Cur_Media_Length))
		...
		hook.Add("Think", "Menu Time Seekto bar Think", function()
			...
			if not PlayMP.MainMenuPanel:IsValid() then hook.Remove("Think", "Menu Time Seekto bar Think") return end
			if PlayMP.Player.Cur_play_time == nil then return end
			if PlayMP.Player.Cur_Media_Length == nil then return end
				...
				--	end
					...
					PlayMP:AddActionButton( ButtonPanel, PlayMP:Str("QueueRequest"), Color( 42, 205, 114 ), ButtonPanel:GetWide() - 200, 10, 100, 30, function() 
						...
						if SS != nil and SM != nil and ES != nil and EM != nil then
							...
							PlayMP:AddQueue( tostring(URLEntry:GetValue()), queueRequest.startTime, queueRequest.endTime, false, PlayMP:GetSetting( "removeOldQueue" ) )
	...
	if PlayMP.NotchInfoPanel != nil and PlayMP.NotchInfoPanel:Valid() then -- 정보패널에 알림을 표시하기 전 정보패널 존재 확인
		...
		if ivbas != true then
			...
			if type == "warning" then
				PlayMP.NotchInfoPanel:MoveTo( ScrW() * 0.55 - ScrW() * 0.2 -10, 20, 0.05, 0, -1, function()
				PlayMP.NotchInfoPanel:MoveTo( ScrW() * 0.55 - ScrW() * 0.2 +10, 20, 0.05, 0, -1, function()
				PlayMP.NotchInfoPanel:MoveTo( ScrW() * 0.55 - ScrW() * 0.2 -10, 20, 0.05, 0, -1, function()
				PlayMP.NotchInfoPanel:MoveTo( ScrW() * 0.55 - ScrW() * 0.2 +10, 20, 0.05, 0, -1, function()
				PlayMP.NotchInfoPanel:MoveTo( ScrW() * 0.55 - ScrW() * 0.2, 20, 0.05, 0, -1, function()
				end)
				end)
				end)
				end)
				...
				PlayMP.NotchInfoPanel:MoveTo( ScrW() * 0.55 - ScrW() * 0.2, 20, 1, 0, -1)
			end
		...
		NoticePanel:AlphaTo( 255, 0.5, 0, function()
			NoticePanel:AlphaTo( 0, 5, 9.5, function()
				...
				if ivbas != true then
					...
					PlayMP.NotchInfoPanel:MoveTo( ScrW() * 0.55 - ScrW() * 0.2, 0, 1, 0, -1, function() 
					end)
				end
